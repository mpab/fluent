...
ostream &operator<<(ostream &o, const Instruction *i) {
  o << "[";
...
ostream &operator<<(ostream &o, const NodeInfo &n) {
  if (!n.n) {
    return o << FMT16 << reinterpret_cast<const void *>(n.n);
  }

  o << FMT16 << reinterpret_cast<const void *>(n.n);

  auto i = dynamic_cast<Instruction *>(const_cast<Node *>(n.n));

  if (i) {
    o << i->type_name() << ": '" << opcode_name(i->opcode) << "' ";
